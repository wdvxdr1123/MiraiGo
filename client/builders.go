...
// OidbSvc.0x899_0
func (c *QQClient) buildGroupMemberListPacket(groupCode uint64) (uint16, []byte) {
	seq := c.nextSeq()
	body := &oidb.D899ReqBody{
		GroupCode: &groupCode,
		StartUin: proto.Uint64(0),
		IndentifyFlag: proto.Uint32(2),
		MemberList: &oidb.D899MemberList{
			MemberUin: proto.Uint64(0),
			Privilege: proto.Uint32(1),
		},
	}
	b, _ := proto.Marshal(body)
	req := &oidb.OIDBSSOPkg{
	...
	payload, _ := proto.Marshal(req)
	packet := packets.BuildUniPacket(c.Uin, seq, "OidbSvc.0x899_0", 1, c.OutGoingPacketSessionId, EmptyBytes, c.sigInfo.d2Key, payload)
	return seq, packet
}